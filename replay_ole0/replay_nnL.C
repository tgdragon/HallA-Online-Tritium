...
void replay_coinc_new(Int_t runnumber=0,Int_t numevents=0,Int_t fstEvt=0,Bool_t QuietRun = kTRUE, Bool_t OnlineReplay =kFALSE, Bool_t bPlots = kFALSE, Bool_t autoreplay = kFALSE){
	...
	HRSL->AddDetector( new HallA::FadcScintillator("s2"  , "S2 Scintillator" ));
	...
	HRSR->AddDetector( new HallA::FadcScintillator("s2", "S2 Scintillator - FADC" ));
	...
	HRSR->AddDetector( new TriFadcAerogelC("a1" , "Aerogel counter -FADC"        )); // TG
	HRSR->AddDetector( new TriFadcAerogelC("a2" , "Aerogel counter -FADC"        )); // TG
