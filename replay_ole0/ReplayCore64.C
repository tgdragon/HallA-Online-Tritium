...
cout << endl 
  ...
  //HACK
  analyzer->AddInterStage( new HallA::TwoarmVDCTimeCorrection("S2LRTimeDiff","Time difference S2L-S2R",
                                                              "R.s2","L.s2") );
  ...
  TString oldfilename="";
  THaRun *oldrun=0, *run=0, *runlist[30]={0};Int_t runidx=0;
  Bool_t exit=false;
  ...
  for (Int_t nsplit=0;!exit;nsplit++)
	...
	run->SetFirstEvent(FirstEventNum);
        ...
        Int_t final_nev = FirstEventNum;
	try {
	  final_nev = analyzer->Process(run); 
	}
	catch( exception& e ) {
