...
Int_t Tritium_HRS::TrackTimes( TClonesArray* Tracks )
{
 ...
 TriFadcXscin *s0_scin=dynamic_cast<TriFadcXscin*>(GetDetector("s0"));
 if(!s0_scin) {
   cerr << "No s0 detector ... twonk" << endl;
   exit(1);
 }
 //OLDOLD TriFadcScin *s2_scin=static_cast<TriFadcScin*>(GetDetector("s2"));
 THaScintillator *s2_scin=dynamic_cast<THaScintillator*>(GetDetector("s2"));
 if(!s2_scin) {
   cerr << "No s2 detector ... twonk" << endl;
   exit(1);
 }
 THaTrack *track=0;
...
{ track=static_cast<THaTrack*>(Tracks->At(i));
 ...
 // cout<<"1="<<s2pad<<endl;
  ...
  //OLDOLD Double_t s2time=(s2_scin->GetTimes())[s2pad];
  Double_t s2time = s2_scin->GetPad(s2pad).time;
 //cout <<"2="<<s2time<<endl;
