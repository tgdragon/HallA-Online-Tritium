void z_center(Int_t run=0, Double_t current=0){
  ...
  TH2D *zvx = new TH2D("zvx","Z vs Raster X",36,62000,93000,300,-.015,.015);

  TString cut;

  if(true){
    cut = "(evRightdnew_r*0.0003297)>";
    cut += current;
    rootfile->Draw("rpr.z:Lrb.Raster2.rawcur.x>>zvx",TCut(cut));
  }else{
  ...
  TF1 *gaus_fit = new TF1("Gaussian","gaus",-.015,.015);
  //TObjArray arr;
