...
#define RIGHT_ARM_CONDITION true
#define LEFT_ARM_CONDITION false

#include "TString.h"

void hole_fit(Int_t r1=0, Int_t r2=0, Double_t r2_current=0, Double_t x_true=0.){
  TString rpath = "/chafs1/work1/tritium/tmp_data/eeL";
  Int_t run = r1;
  if(run==0){
  ...
  TF2 *ell_fit2 = new TF2("ell_fit2","([0]/(1. + exp(-1. * [5] * ((([1]*([2]-x))^2 + ([3]*([4]-y))^2)-1.)))) + [6]",58000,87000,61000,79000);
  ...
  ell_fit2->SetParLimits(2,60000,85000);
  ell_fit2->SetParLimits(3,.00001,.0005);
  ell_fit2->SetParLimits(4,63000,77000);
  ...
  TH2F *R2 = new TH2F("R2","Raster 2 Carbon Hole",60,55000,90000,60,58000,82000);
...
//    cout << endl << "test " << TMath::Exp(-1. * ell_fit->GetParameter(5) * (TMath::Power(x_slope/x_true,2.) - 1.)) << endl;
  ...
  TH1F *r1xcurr = new TH1F("r1xcurr", "Raster 1-X Current vs ADC Channel", 1000, 45000, 100000);
  TH1F *r1ycurr = new TH1F("r1ycurr", "Raster 1-Y Current vs ADC Channel", 1000, 20000, 120000);
  TH1F *r2xcurr = new TH1F("r2xcurr", "Raster 2-X Current vs ADC Channel", 1000, 45000, 100000);
