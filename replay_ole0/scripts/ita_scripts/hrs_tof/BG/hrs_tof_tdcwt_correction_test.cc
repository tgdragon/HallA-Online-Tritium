...
void hrs_tof_tdcwt_correction_test(){
...
//-------- TTree data input ---------------//
 ...
 for(Int_t i = 93500; i<93503;i++) /// these run goes 93495 to 93504
 {
...
ffit[i][j]=new TF1(Form("ffit[%d][%d]",i,j),"[0]*(1./sqrt(x)-1./sqrt([1]))",min_adc,max_adc);
 ...
 //ffit[i][j]->SetParLimits(0,0.6*fit_ini(i,j,0),1.4*fit_ini(i,j,0));   
...
cut_time[16]="R.s0.lt_c>0 && R.s0.rt_c>0";
 ...
 htdc_m[i]=new TH1F(Form("htdc_m[%d]",i),Form("TDC mean hist w/ TW correction ch %d",i),1000,-1.0e-8,1.0e-8);
...
double range_para(int i,int j){
 ...
 //param 4 min_adc, 5 max_adc //
  ...
  //===== Set Parameters ========//
  par[8][0]=0.18e-6, par[8][1]=-0.22e-6, par[8][2]=0.18e-6;
  par[16][0]=0.180e-6, par[16][1]=0.23e-6, par[16][2]=-1.8e-7, par[16][3]=2.5e-7;
  par[ch][4]=500.0, par[ch][5]=5000;//ADC
  //============================//
  for(int k=0;k<chmax;k++){
