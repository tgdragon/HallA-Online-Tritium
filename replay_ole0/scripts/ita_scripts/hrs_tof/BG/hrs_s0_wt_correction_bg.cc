 ...
 */
 ...
 T->Project("tp[16][0]","R.s0.rt_c:R.s0.ra_c",cut_trig &&cut_time[i]);//Right PMT
    T->Project("tp[16][1]","R.s0.lt_c:R.s0.la_c",cut_trig &&cut_time[i]);//Left PMT
     ...
     //  ffit[16][0]->SetParameter(0,fit_ini(16,0,0));   
     //   ffit[16][0]->SetParameter(1,fit_ini(16,0,1));
     //    ffit[16][1]->SetParameter(0,fit_ini(16,1,0));   
     //   ffit[16][1]->SetParameter(1,fit_ini(16,1,1));
 ...
 //------ Project hist TDC vs ADC w/ Correction ----------------//
    ...
    //----- ADC vs TDC w/ correction----------//
                         cw[i]->Divide(1,2);
                         cw[i]->cd(1);
			 htdc_adc_c[16][0]->Draw();
                         cw[i]->cd(2);
			 htdc_adc_c[16][1]->Draw();
 ...
 T->Project("htdc[16][1]","R.s0.lt_c",cut_trig && cut_time[16]);   		    
			 ...
			 ctdc[i]->Divide(2,2);
			 ctdc[i]->cd(1);
			 //		 htdc[16][0]->SetFillColor(1);
			 // htdc_c[16][0]->SetFillColor(2);
			 //	 htdc[16][0]->GetXaxis()->SetRangeUser(min_tof,max_tof);
                         htdc[16][0]->Draw();
			 // htdc_c[i][0]->Draw("same");
                         ctdc[i]->cd(2);
			 //	 htdc[16][1]->SetFillColor(1);
			 // htdc_c[16][1]->SetFillColor(2);
 ...
 */		
                        ...
                        //--- w/ correction --//
			 ctdc[i]->cd(3);
			 //			 htdc_c[16][0]->GetXaxis()->SetRangeUser(min_tof_c,max_tof_c);
                         htdc_c[16][0]->Draw();
                         ctdc[i]->cd(4);
			 //			 htdc_c[16][1]->GetXaxis()->SetRangeUser(min_tof_c,max_tof_c);
			 htdc_c[16][1]->Draw();
