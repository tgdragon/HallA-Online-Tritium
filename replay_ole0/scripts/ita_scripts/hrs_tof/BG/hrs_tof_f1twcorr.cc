...
// time walk correction R-HRS S2 F1TDC
...
//-------- TTree data input ---------------//
 ...
 for(Int_t i = 94000; i<94004;i++) /// these run goes 93495 to 93504
 {
   // T->Add(Form("/adaqfs/home/a-onl/tritium_work/itabashi/HallA-Online-Tritium/replay/Rootfiles/pass1/tritium_%d*.root",i));
   T->Add(Form("/u/home/itabashi/nnL/Tohoku_github/HallA-Online-Tritium/replay/t2root/Rootfiles/tritium_%d.root",i));
 ...
 //====== Fill w/ Time-Walk Correction ======//
  ...
  for(int i=0;i<chmax;i++){
    if(i==ch){
      ...
      min_tof[i]=-1.0e-7;   max_tof[i]=0.0;//1.0e-6;
 ...
 ffit_r[i]=new TF1(Form("ffit_r[%d]",i),"[0]*(1./sqrt(x)-1./sqrt([1]))",min_adc[i],max_adc[i]);
 ffit_l[i]=new TF1(Form("ffit_l[%d]",i),"[0]*(1./sqrt(x)-1./sqrt([1]))",min_adc[i],max_adc[i]);

      //------------ w/ Time-Walk Correction Parameters ----------------------//
      min_tof_c[i]=-1.0e-7;   max_tof_c[i]=0.0;//1.0e-6;
      bin_tof_c[i]=(max_tof_c[i]-min_tof_c[i])/tdc_time;  bin_tof_c[i]=(int)bin_tof_c[i];
      min_adc_c[i]=0.0;   max_adc_c[i]=500.;
 ...
 if(rarm){
  ...
  if(tdc_cut[i] && trig_cut)htof_adc_r[i]->Fill(S2r_a[i],tof[i]);
  if(tdc_cut[i] && trig_cut)htof_adc_l[i]->Fill(S2l_a[i],tof[i]);
 ...
 ffit_r[i]->SetParameter(0,fit_ini(i,0,0));   
 ffit_r[i]->SetParameter(1,fit_ini(i,0,1));   
 ffit_l[i]->SetParameter(0,fit_ini(i,1,0));   
 ffit_l[i]->SetParameter(1,fit_ini(i,1,1));   
...
// ffit[i][j]->FixParameter(0,fit_ini(i,j,0));   
 ...
 //------FBUS ADC ----//
  ...
  corr_r[i]=twp_r[i][0]*(1./S2r_a[i]-1./sqrt(twp_r[i][1]));
  corr_l[i]=twp_l[i][0]*(1./S2l_a[i]-1./sqrt(twp_l[i][1]));
  tof_c[i]=(S2l_t[i]+S2r_t[i])/2.0-(S0a_t+S0b_t)/2.0-0.5*corr_r[i]-0.5*corr_l[i];
 
  //------- Fill Hist w/ Time-Walk Correction ------//
  if(tdc_cut[i] && trig_cut)htof_c[i]->Fill(tof_c[i]);
  if(tdc_cut[i] && trig_cut)htof_adc_rc[i]->Fill(S2r_a[i],tof_c[i]);
  if(tdc_cut[i] && trig_cut)htof_adc_lc[i]->Fill(S2l_a[i],tof_c[i]);
