...
//========== Fill TOF vs ADC w/o Time-Walk CorrectionHist ==============//
 ...
 if(tdc[i][0]>0 && tdc[i][1]>0)cut_time[i]=true;
  ...
  if(j==0 && cut_trig && cut_time[i] && cut_time[16])htof[i]->Fill(tof[i]);
  //------- TOF vs ADC 2D Hist ----------------------//  
...
int main(int argc, char** argv){
 ...
 if(print)twcorr->print();
 if(param)twcorr->param();
 if(draw)twcorr->draw();
 if(print)twcorr->print();
