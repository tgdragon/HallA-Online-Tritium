...
int main(int argc, char** argv){
  ...
  if(nf<1){
    ...
    if(tflag==5){
      ...
      sprintf(newfname,"./nnL/LHRS_single_dragon2/tri_Lsingle_%d.root",run);
    }
    ...
    if(tflag==5){
      ...
      sprintf(newfname,"./nnL/RHRS_single_dragon2/tri_Rsingle_%d_%d.root",run,nf);
    }
    else if(tflag==1){
      sprintf(newfname,"./nnL/LHRS_single_dragon2/tri_Lsingle_%d_%d.root",run,nf);
    }
  ...
  t1->SetBranchAddress("DR.T1", &trig1    ); // Left
  t1->SetBranchAddress("DR.T4", &trig4    ); // Right
  t1->SetBranchAddress("DR.T5", &trig5    ); // Coin
  ...
  for(int i=0 ; i<ent ; i++){
	...
	//double vzt = (rvz_cor - Ztm)/Ztr;
	double vzt = (lvz_cor - Ztm)/Ztr;
