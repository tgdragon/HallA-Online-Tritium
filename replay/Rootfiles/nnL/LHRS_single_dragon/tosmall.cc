/*
  ...
  This was modifed from nudeL.cc
...
int tosmall(int run=111721, int nf=0, int tflag=1){
  int dataflag = 1;
  if (run>111400){
    dataflag = 2;
  ...
  if(nf<1){
    sprintf(inputfname,"./20190202/tri_Lsingle_%d.root",run);
    sprintf(newfname,"./20190202/Lsmall_%d.root",run);
    if(tflag==5){
    ...
    sprintf(inputfname,"./20190202/tri_Lsingle_%d_%d.root",run,nf);
    ...
    if(tflag==5){
      ...
      sprintf(newfname,"./20190202/Lsmall_%d_%d.root",run,nf);
    }
  ...
  //t1->SetBranchAddress("L.tr.vz2", &lvz);
  ...
  t1->SetBranchAddress("Lrb.x2", &rast_x2);
  ...
  tnew->Branch("Xpt", &th2,  "Xpt[100]/D");
  tnew->Branch("Ypt", &ph2,  "Ypt[100]/D");
  tnew->Branch("Zt",  &lvz,  "Zt[100]/D");
  ...
  tnew->Branch("XFP",   &l_x_fp,  "XFP[100]/D");
  tnew->Branch("YFP",   &l_y_fp,  "YFP[100]/D");
  tnew->Branch("XpFP",  &l_th_fp, "XpFP[100]/D");
  tnew->Branch("YpFP",  &l_ph_fp, "YpFP[100]/D");
  ...
  tnew->Branch("Xt", &rast_x2, "Xt/D");
  tnew->Branch("Yt", &rast_y,  "Yt/D");
  ...
  for(int i=0 ; i<ent ; i++){
	...
	lvz[0] = lvz[0] + rast_x2/tan(13.2/180.*3.14159); // raster correction
