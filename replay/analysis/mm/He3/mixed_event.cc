...
const double m_3H = 2.80839133; // He3 mass
const double m_nnL = 1.875612762 + 1.115683; // d+L
//
//const double m_3H   = 2.80892086; // 3H
...
void mixed_event(){
  TFile* f1 = new TFile("../He3_20191208.root");
  ...
  for(int i=0 ; i<ent ; i++){
	...
	if(ctime_selection==true){
	  ...
	  mm = CalcMM(hallap, par_ep, par_k, m_3H);
	...
	for(int j=0 ; j<(int)nmix ; j++){
	  ...
	  mixed_mm = CalcMM(hallap, par_ep, par_k, m_3H);
