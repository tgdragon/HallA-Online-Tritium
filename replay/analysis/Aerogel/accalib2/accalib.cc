/*
  "accalib.cc"
  Pedestal and gain parameters calibration for A2
  
  Toshiyuki Gogami, 6 Oct 2018
*/

const int n = 26; // Number of channels for A2
...
int accalib(int ch = 10){
  ...
  if (iwanttoprint==true){
    char pname[500];
    sprintf(pname,"./figures/fit_a2adc_%d.png",ch);
    c1->Print(pname);
    sprintf(pname,"./figures/fit_a2adc_%d.eps",ch);
    c1->Print(pname);
  }
...
void CreateHistrootF(){ 
  ...
  //const int n = 24;
  double adc[n];
  t1->SetBranchAddress("R.a2.a_c", &adc);
